...
// 新增命令行参数
// 写带宽限制（KB/s，-1=无限制）
static int FLAGS_write_bandwidth = -1; 

// 读带宽限制（KB/s，-1=无限制）
static int FLAGS_read_bandwidth = -1; 

// 是否启用CPU亲和性绑定
static bool FLAGS_thread_affinity = false;
...
struct ThreadState {
  int tid;                          // 线程ID（0~n-1）
  Random rand;                      // 线程独立的随机数生成器
  Stats stats;                      // 线程统计信息
  SharedState* shared;              // 共享状态
  BandwidthController* write_bw;    // 写带宽控制器（Owns）
  BandwidthController* read_bw;     // 读带宽控制器（Owns）

  // 构造函数：支持传入带宽限制（单位：KB/s）
  ThreadState(int index, int seed, int write_bw_kbps, int read_bw_kbps)
      : tid(index),
        rand(seed),
        shared(nullptr),
        write_bw(write_bw_kbps > 0 ? 
                 new BandwidthController(write_bw_kbps * 1024) : nullptr), // KB→B
        read_bw(read_bw_kbps > 0 ? 
                new BandwidthController(read_bw_kbps * 1024) : nullptr) {}
...
class BandwidthController {
 public:
  explicit BandwidthController(int64_t max_bytes_per_sec)
      : max_bytes_per_sec_(max_bytes_per_sec),
        start_time_(g_env->NowMicros()),
        bytes_used_(0) {
    assert(max_bytes_per_sec <= 0 || max_bytes_per_sec > 0); // 0表示无限制
  }

  // 记录本次操作使用的字节数，并根据需要休眠线程以限制速率
  void Throttle(int64_t bytes) {
    if (max_bytes_per_sec_ <= 0) return; // 未启用带宽限制

    bytes_used_ += bytes;
    uint64_t now = g_env->NowMicros();
    uint64_t elapsed_us = now - start_time_;

    // 计算当前已用时间内允许的最大字节数
    int64_t allowed_bytes = static_cast<int64_t>(
        (elapsed_us / 1e6) * max_bytes_per_sec_);

    // 如果已用字节数超过允许值，计算需要休眠的时间
    if (bytes_used_ > allowed_bytes) {
      double required_time = bytes_used_ * 1.0 / max_bytes_per_sec_;
      uint64_t required_us = static_cast<uint64_t>(required_time * 1e6);
      uint64_t sleep_us = required_us - elapsed_us;
      if (sleep_us > 0) {
        g_env->SleepForMicroseconds(sleep_us);
      }
    }
  }

 private:
  const int64_t max_bytes_per_sec_; // 最大允许字节/秒（<=0表示无限制）
  uint64_t start_time_;             // 本次统计周期的起始时间（微秒）
  int64_t bytes_used_;              // 本次统计周期内已使用的字节数
};
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < num_threads; i++) {
      int seed = FLAGS_threads == 1 ? 301 : (301 + i); // 线程独立种子
      threads.push_back(new ThreadState(
          i, seed, FLAGS_write_bandwidth, FLAGS_read_bandwidth));
    }
    ...
    // 线程函数中添加CPU亲和性绑定（在 ThreadBody 中）
    static void ThreadBody(void* arg) {
      ThreadArg* targ = static_cast<ThreadArg*>(arg);
      ThreadState* thread = targ->thread;

      // 启用CPU亲和性（绑定到不同核心）
      if (FLAGS_thread_affinity) {
    #ifdef _WIN32
      DWORD_PTR mask = 1ULL << (thread->tid % std::thread::hardware_concurrency());
      SetThreadAffinityMask(GetCurrentThread(), mask);
    #else
      cpu_set_t cpuset;
      CPU_ZERO(&cpuset);
      CPU_SET(thread->tid % std::thread::hardware_concurrency(), &cpuset);
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    for (int i = 0; i < num_; i += entries_per_batch_) {
      ...
      // 应用写带宽控制（关键修改）
      if (thread->write_bw) {
        thread->write_bw->Throttle(total_bytes);
      }
      if (!s.ok()) {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_db = argv[i] + 5;
    } else if (sscanf(argv[i], "--write_bandwidth=%d%c", &n, &junk) == 1) {
      FLAGS_write_bandwidth = n;
    } else if (sscanf(argv[i], "--read_bandwidth=%d%c", &n, &junk) == 1) {
      FLAGS_read_bandwidth = n;
    } else if (sscanf(argv[i], "--thread_affinity=%d%c", &n, &junk) == 1) {
      FLAGS_thread_affinity = (n != 0);
    }
    else {
